...
def main():
    ...
    def lendis():
        '''main.<locals>.lendis'''
        __qualname__ = 'main.<locals>.lendis'
        ...
        def getBal(self, root):
            if not root:
                return 0
            return None.getHeight(root.l) - self.getHeight(root.r)
    ...
    def on_focus_out(entry, placeholder):
        if entry.get() == '':
            ...
            entry.configure('disabled', **('state',))
    ...
    inp = Entry(root, 33, 3, RIDGE, **('width', 'borderwidth', 'relief'))
    inp.grid(5, 0, 'w', 15, **('pady', 'row', 'sticky', 'padx'))
    inp.insert(0, 'Input Secret Key')
    inp.configure('disabled', **('state',))
    x_focus_in = None(None, (lambda x = None: on_focus_in(inp)))
    submitbutton = None(None, None, None, (lambda : submit(troot)), 'red', 'white', 3, RIDGE, **('text', 'width', 'command', 'bg', 'fg', 'borderwidth', 'relief'))
    submitbutton.grid(1, 'w', 15, 5, **('row', 'sticky', 'padx', 'pady'))
    text = StringVar()
    text.set(999 - turn)
    textbox = Label(root, text, 'center', 13, **('textvariable', 'justify', 'width'))
    textbox.config(('Courier', 30), **('font',))
    textbox.grid(5, 2, 'w', **('pady', 'row', 'sticky'))
    root.mainloop()
